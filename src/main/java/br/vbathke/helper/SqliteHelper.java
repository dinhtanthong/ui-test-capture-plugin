            ...
            try{
            	try{
	    			...
	    			while (rs2.next()) {
	    			     ...
	    			     data_tb_test.append("insert into tb_test(id,id_job, test, test_class, status, status_description) values('"+id+"','"+id_job+"','"+test+"','"+test_class+"','"+status+"','"+status_description+"');\n");
	    			}
	    			rs2.close();
	    			getConn().createStatement().close();
