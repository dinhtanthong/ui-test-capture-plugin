        ...
        try {
			...
			if(getId() <= 0){
				if(!getTest().equals("") || getIdJob() <= 0){
		            stmt.executeUpdate("INSERT INTO tb_test(id_job, test, test_class, status, status_description) "
		            		+ "values('"+this.getIdJob()+"','"+this.getTest()+"','"+this.getTestClass()+"','"+this.getStatus()+"','"+this.getStatusDescription()+"');");
				}
			}else{
